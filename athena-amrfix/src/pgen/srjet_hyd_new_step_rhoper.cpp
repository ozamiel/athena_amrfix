...
//    }
 // }
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    AthenaArray<Real> bb;
    bb.NewAthenaArray(ku-kl+1,ju-jl+1,iu-il+1);
...
void JetInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
 ...
 //std::cout << "kl = " << kl << "\n";
    ...
    //std::cout << gamma << " " << Psi << " " << atwd_sm << " " << bern_sm << " " << r_0 << " " << r << " " << z << " " << pco->x2v(j) << "\n";
...
//      }
	...
	for (int i=il; i<=iu; ++i) {
      ...
      Real r_0 = r0_r_z(r,z);
	  Real smfnc = SmoothStep((r_0 - r_jet)/dr_jet);
	  b.x2f(kl-k,j,i) = (b_0*a*r_0/(a*a+r_0*r_0)) * smfnc; //Setting Bphi = R/a * Bz - force-free solution (total pressure-free in our case).
	}
      }
    }
